...
"""
This codes parses the  collaborators submission manifest, 
creates a  CARRIER ID for each unique Sample, does some degree of
validation and creates three excel files based on 
Jenna's database schema
"""
...
carrier_headers = ('CARRIERS ID', 'Sub_Sample Name', 'Sub_Individual ID', 'Sub_Gender',
                   'Sub_Sample Status','Sub_Pedigree', 'Sub_Mother ID', 'Sub_Father ID',
...
def run(logger_filename, submission_manifest, carrier_headers, carrier_output_table,
    ...
    cast_plate_dict, contact_id = create_cast_plate(manifest)
    fields_header = get_data_fields_header(manifest) 
    index, info_index = data_fields_index(manifest)
    carrierID_field, caid_plate_field, carrier_sample =  sample_data(manifest, fields_header, index, info_index)
    ...
    carrier_tuple_output, caid_tuple_output, cast_tuple_output = create_tuple_output(carrierID_field,
                                                                                     caid_plate_field,
                                                                                     cast_plate_dict,
                                                                                     create_carrier_ID,
                                                                                     sample_number)
...
def create_index(sheet, field, x, y, z):
    """
    pre-processing cast plate fields
    """
    cast_plate = []
...
def create_cast_plate(sheet):
    """
    creating a cast plate dictionary, assuming positions
    will be constant as it is a fixed manifest
    """
    cast_plate_dict = {}
    ...
    comb = Plate_Barcode + contact_id + contact_person + contact_email + \
            project_type + plate_name + plate_description
    cast_plate_dict = dict(comb)
    return cast_plate_dict, contact_id



class cast_plate(object):

    y = 2
    z = 0

    def __init__(self, plate_barcode, contact_id, contact_person,
                 project_type, plate_name, plate_description):
    ...
    def create_index(sheet, self, x, y, z):
        """
        pre-processing cast plate fields
        """
        cast_plate = []
...
def get_data_fields_header(sheet):
    """
    Pre-processing header to remove special characters
    """
    ...
    return new_header
#TODO: Throw regex on top as global, as it's being used way to often
...
def data_fields_index(sheet):
    ...
    for rowidx in range(sheet.nrows):
        ...
        if row[0].value =="Coord *":
            ...
            if "Plate Barcode" in row[0].value:
                header_index.append((rowidx, int(rowidx)+3))
        
    return data_index, header_index



def sample_data(sheet, new_header, data_index, header_index):
    """
    ...
    for i in data_index:
        ...
        for l in target:
            if l.Sample_Name != '':
                ...
                carrierID_dict[l.Sample_Name] = [l.Individual_ID, l.Gender,
                                                 l.Sample_Status,
                                                 l.Pedigree, l.Mother_ID,
                                                 l.Father_ID, l.Disease_Type,
                                                 l.Race, l.Ethnicity]
                caid_plate_dict[l.Sample_Name] = [l.Coord, l.Vol_ul, l.Conc_ngul,
                                                  l.Alias, l.Site_of_Origin,
                                                  l.Tissue_Source, l.Sample_Blank,
                                                  l.Default_Control, l.Comment]
    for i in header_index:
        for x, row_index in enumerate(range(i[0], i[1])):
            if x % 3 == 0:
                info_dict_list.append({"Plate_Barcode":sheet.cell(row_index, 2).value})
            elif x % 3 == 1:
                info_dict_list.append({"Plate_Name":sheet.cell(row_index, 2).value})
            else:
                x % 3 == 2
                info_dict_list.append({"Plate_description":sheet.cell(row_index, 2).value})
...
def connect_db(logger):
    """
    connecting to the database and grabbing the last carrier ID
    """
    ...
    if not carrier_id_db:
        last_carrier_id = "CA00000000"
...
def generate_carrier_id(Sample_Name, logger, last_carrier_id):
    """
    creating a carrier id for each sample
    """
    ...
    logger.info("number of last interger carrier id %s", non_zero_number)
    carrier_trunk = ["{0:08}".format(num) for num in xrange(int(digit_field) +1, (int(digit_field)+ set_length +1 ))]
    ...
    for carrier_key, carrier_value in  carrierID_dict.items():
        if sample_name_carrier_id[carrier_key]:
            carrier_id_string = ((str(sample_name_carrier_id[carrier_key]),
                                  str(carrier_key), str(",".join(carrier_value[:])),''))
    ...
    for caid_key, caid_value in caid_plate_dict.items():
        if sample_name_carrier_id[caid_key]:
            ...
            caid_tup = caid_id_string[:3] + tuple(caid_id_string[-1].split(",")[:-2])\
                       + tuple([caid_id_string[-1].split(',')[-1]]) + tuple('')
            caid_id_table.append(caid_tup)

    cast_plate_row = (cast_plate_dict['Plate_Barcode'],'' , '',str(cast_plate_dict['Contact_ID_Study_Acronym']),
                      str(cast_plate_dict['Contact_Person']), str(cast_plate_dict['Contact_Email']),
                      str(cast_plate_dict['Project_Type']), str(cast_plate_dict['Plate_Name']),
                      str(cast_plate_dict['Plate_Description']),"","","","","","","","")
...
def generate_excel_output(carrier_output_table, carrier_headers, carrier_id_table,
                          caid_cast_output_table,caid_cast_header,
                          caid_id_table, cast_plate_output_table, cast_plate_header,
                          cast_id_table, current_date, contact_id):
    ...
    out_cont =contact_id[-1][-1]
    output_filename = ''.join("carriers" + "_" + ''.join(out_cont) + "_" + str(current_date) +".xls")
